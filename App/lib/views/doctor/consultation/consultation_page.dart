...
class _DoctorConsultationPageState extends ConsumerState<DoctorConsultationPage>
  ...
  Future<void> _initVisit() async {
    ...
    // Only run if NOT already IN_PROGRESS — backend forbids self-transition.
    if (_apptId > 0 && currentStatus != 'IN_PROGRESS') {
      try {
        if (currentStatus == 'SCHEDULED') {
          await vm.updateAppointmentStatus(_apptId, 'CONFIRMED');
        }
        await vm.updateAppointmentStatus(_apptId, 'IN_PROGRESS');
      } catch (e) {
        debugPrint(
          '⚠️ Status transition failed: ${DoctorViewModel.extractError(e)}',
        );
      }
    }

    // ── Step 2: Start OR resume visit ─────────────────────────────────────────
    // If already IN_PROGRESS, skip startVisit entirely — it will always fail.
    // Go straight to fetching the existing visit.
    if (currentStatus == 'IN_PROGRESS') {
      try {
        final visits = await vm.fetchVisits(_patId);
        final existing = visits.firstWhere(
          (v) =>
              v['appointment_id']?.toString() == _apptId.toString() ||
              v['appointment']?['id']?.toString() == _apptId.toString(),
          orElse: () => visits.isNotEmpty ? visits.last : <String, dynamic>{},
        );
        if (mounted && existing.isNotEmpty) {
          setState(() => _visit = existing);
        }
      } catch (e) {
        debugPrint(
          '⚠️ Could not resume visit: ${DoctorViewModel.extractError(e)}',
        );
      } finally {
    ...
    // Fresh appointment — try to start a new visit normally.
    try {
      ...
      // Fallback: visit may have been created by a race condition — fetch it.
      try {
        final visits = await vm.fetchVisits(_patId);
        final existing = visits.firstWhere(
          (v) =>
        ...
        debugPrint(
          '⚠️ Could not load visit: ${DoctorViewModel.extractError(e)}',
        );
      }
    } finally {
      if (mounted) setState(() => _startingVisit = false);
  ...
  Future<void> _transcribe(String path) async {
    ...
    try {
      ...
      final String txt;
      if (_visitId > 0) {
        txt = await vm.transcribeAudio(
          audioFile: File(path),
          visitId: _visitId,
        );
      } else {
        txt = await vm.transcribeAudioLocal(audioFile: File(path));
      }
  ...
  /// Called by VoiceRecordingWidget when a recording is complete.
  /// Calls transcribeAudioLocal (no DB write) and surfaces the result in the AI tab.
  Future<void> _onVoiceRecorded(String audioPath) async {
    ...
    try {
      ...
      final transcription = await vm.transcribeAudioLocal(audioFile: file);
      ...
      if (transcription.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'AI returned an empty transcription. Please try again.',
            ),
      ...
      if (saved == true && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Voice report saved to patient records.'),
