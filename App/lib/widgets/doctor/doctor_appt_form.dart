...
class _DoctorApptFormState extends State<DoctorApptForm> {
  ...
  // True once visit type is confidently resolved from name/ID data.
  // When false after initState, _loadAndApplyFeeDefaults() attempts
  // fee-based inference as a last resort.
  bool _visitTypeResolved = false;
  ...
  // Fallback types used when the backend returns an empty list.
  // No 'id' field — prevents sending an appointment_type_id that belongs
  // to a different doctor when the real types haven't loaded yet.
  ...
  List<Map<String, dynamic>> get _effectiveTypes =>
      widget.types.isNotEmpty ? widget.types : _fallbackTypes;

  // Finds the appointment type ID in _effectiveTypes that matches the category.
  int? _typeIdForCategory(String category) {
    ...
    for (final t in _effectiveTypes) {
      ...
      if (category == 'revisit') {
        if (n.contains('revisit') || n.contains('follow')) {
          return int.tryParse(t['id'].toString());
        }
      } else {
    ...
    // Pass 2 (consultation only) — fall back to the first non-revisit type.
    // Handles Arabic-named types (e.g. "كشف") that don't contain 'consult'.
    if (category != 'revisit') {
      for (final t in _effectiveTypes) {
        if (!_isRevisitTypeName((t['name'] ?? '').toString())) {
          return int.tryParse(t['id'].toString());
        }
      }
    }
  ...
  bool _isRevisitTypeName(String name) {
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      // Guard against appointment_type being a non-Map (e.g. raw int) which
      // would throw if accessed with ['id'] via the null-safe ?. operator.
      ...
      // ── 3-layer visit type resolution ──────────────────────────────────────
      // Layer 1: explicit name fields (both flat and nested API shapes)
      String rawTypeName = (e['appointment_type_name'] ??
              (e['appointment_type'] is Map
      ...
      // Layer 2: look up name from _effectiveTypes using the stored type ID
      // (covers appointments where the backend only returns appointment_type_id)
      if (rawTypeName.isEmpty && _typeId != null) {
        for (final t in _effectiveTypes) {
          if (int.tryParse(t['id'].toString()) == _typeId) {
            rawTypeName = (t['name'] ?? '').toString();
            break;
          }
        }
      }

      _visitType = _isRevisitTypeName(rawTypeName) ? 'revisit' : 'consultation';
      _typeId ??= _typeIdForCategory(_visitType);

      // _visitTypeResolved = true only when we had actual name/ID data.
      // When false, _loadAndApplyFeeDefaults() will attempt fee-based inference.
      _visitTypeResolved = rawTypeName.isNotEmpty;
      ...
      _typeId = _typeIdForCategory(_visitType);
      _visitTypeResolved = true; // new appointments always start at the default
    }
  }
  ...
  Future<void> _loadAndApplyFeeDefaults() async {
    ...
    setState(() {
      ...
      if (widget.existing == null) {
        // New appointment: set fee from doctor's configured default
        _feeCtrl.text = _visitType == 'revisit'
            ? _revisitDefaultFee.toStringAsFixed(0)
            : _consultDefaultFee.toStringAsFixed(0);
      } else if (!_visitTypeResolved) {
        // Layer 3 — fee-based inference (last resort).
        // Runs only when both name and ID resolution failed, meaning the
        // backend returned no usable type data for this appointment.
        // The stored fee is the most reliable remaining signal.
        final storedFee =
            double.tryParse((widget.existing!['fee'] ?? '').toString()) ?? 0;
        if (storedFee > 0) {
          // Prefer API type default_fee over local prefs (cross-device safe)
          for (final t in _effectiveTypes) {
            final typeFee =
                double.tryParse((t['default_fee'] ?? '').toString()) ?? 0;
            if (typeFee > 0 && (typeFee - storedFee).abs() < 0.01) {
              final name = (t['name'] ?? '').toString();
              _visitType = _isRevisitTypeName(name) ? 'revisit' : 'consultation';
              _typeId ??= int.tryParse(t['id'].toString());
              _visitTypeResolved = true;
              break;
            }
          }
          // Fall back to locally saved fee defaults
          if (!_visitTypeResolved) {
            if ((storedFee - _revisitDefaultFee).abs() < 0.01) {
              _visitType = 'revisit';
              _typeId ??= _typeIdForCategory('revisit');
              _visitTypeResolved = true;
            } else if ((storedFee - _consultDefaultFee).abs() < 0.01) {
              _visitType = 'consultation';
              _typeId ??= _typeIdForCategory('consultation');
              _visitTypeResolved = true;
            }
          }
        }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // ── show inline, using this form's own context (not widget.snack,
      //    which is bound to the parent page's context and would queue the
      //    SnackBar behind the still-open sheet) ─────────────────────────────
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() {
                      _visitType = 'consultation';
                      _typeId = _typeIdForCategory('consultation') ?? _typeId;
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() {
                      _visitType = 'revisit';
                      _typeId = _typeIdForCategory('revisit') ?? _typeId;
...
class DoctorPatientSearchField extends StatefulWidget {
  ...
  final bool locked;
  // ── show a red border when validation failed (no patient selected) ──────
  final bool error;
