...
class _AdminUsersPageState extends ConsumerState<AdminUsersPage>
  ...
  Widget build(BuildContext context) {
    ...
    final currentUserEmail =
        (ref.watch(authProvider).user?.email ?? '').toLowerCase();

    return Column(
      children: [
        _buildHeader(context, at, vm, state),
        _buildTabBar(at),
        Expanded(child: _buildBody(context, at, state, vm, currentUserEmail)),
      ],
    );
  }
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (ctx, i) {
          ...
          return _buildUserCard(context, at, users[i - 1], vm, currentUserEmail);
        },
      ),
    );
  }
  ...
  Widget _buildUserCard(
    ...
    final isSelf = user.email.toLowerCase() == currentUserEmail;
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final err = await vm.toggleUserStatus(user);
                if (!mounted) return;
                messenger.showSnackBar(
                  SnackBar(
                    content: Text(
                      err ?? 'Account ${action.toLowerCase()}d successfully.',
                    ),
                    backgroundColor: err == null ? _T.success : _T.urgent,
...
class _SelfBadge extends StatelessWidget {
  ...
  const _SelfBadge({required this.at});
