...
class DoctorViewModel extends StateNotifier<DoctorState> {
  ...
  /// Transcribes a voice recording via AIService directly.
  /// No visitId required — DB saving is intentionally skipped.
  Future<String> transcribeAudioLocal({required File audioFile}) async {
    final r = await AIService.transcribeReport(audioFile);

    debugPrint('🎙️ AI RAW RESPONSE: $r');

    // If the API returns a single text field, use it directly.
    final direct =
        r['transcription'] ??
        r['text'] ??
        r['content'] ??
    ...
    // The API returns a structured report (symptoms, diagnosis, etc.).
    // Format all fields into a readable medical report.
    final buffer = StringBuffer();
    const skip = {'error', 'status', 'code', 'message', 'id'};

    for (final entry in r.entries) {
      final key = entry.key.toString();
      final value = entry.value;
      if (skip.contains(key.toLowerCase())) continue;
      if (value == null) continue;

      final valueStr = value is List
          ? (value as List).map((e) => e.toString()).join(', ')
          : value.toString().trim();
  ...
  Future<void> saveVoiceReport({
    ...
    await ApiService.createMedicalReport({
      ...
      'title': title,
      'content': content,
      'report_type': 'voice_transcription',
      'status': 'final',
    });
  }
