...
class ApiService {
  ...
  // ── Re-entrancy guard ─────────────────────────────────────────────────────
  // When the app starts, loadAll() fires fetchPatients + fetchAppointments +
  // fetchDoctors in parallel.  If the stored token is expired or missing,
  // all three requests return 401 simultaneously.  Without this guard, each
  // 401 independently calls clearToken() + _onUnauthorized() — causing
  // multiple Navigator.pushAndRemoveUntil() calls on an already-torn-down
  // navigator tree, which is why the UI gets stuck on the doctor/assistant
  // screen with no token instead of landing cleanly on LoginPage.
  //
  // The guard ensures only the FIRST concurrent 401 triggers the full logout.
  // All subsequent parallel 401s return immediately (the credentials are
  // already cleared; there's nothing left to do).
  //
  // resetUnauthorizedGuard() must be called from AuthViewModel.login() after
  // a successful re-authentication so future token-expiry 401s work correctly.
  ...
  /// Resets the re-entrancy guard after a successful login.
  /// Call this from AuthViewModel after saveToken() completes so that a
  /// future token expiry (days/weeks later) can trigger logout again.
  static void resetUnauthorizedGuard() {
    _handlingUnauthorized = false;
  }

  /// Clears all persisted auth data and fires the unauthorized callback.
  /// Protected by [_handlingUnauthorized] so only the first concurrent
  /// 401 triggers the full logout — subsequent parallel 401s are dropped.
  static Future<void> _handleUnauthorized() async {
    if (_handlingUnauthorized) {
      print('🔐 401 received — logout already in progress, skipping.');
      return;
    }
    ...
    // Guard stays true until resetUnauthorizedGuard() is called on next login.
  }
  ...
  static Future<List<dynamic>> getPatients({
    ...
    // doctor_id filter: used by assistants so the backend returns only
    // the linked doctor's patients regardless of JWT scope.
